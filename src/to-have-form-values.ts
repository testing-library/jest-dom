...
function getFormValue(container: HTMLFormElement, name: string) {
  const elements = [
...
function getAllFormValues(container: HTMLFormElement): Record<string, string> {
...
type ExpectedValue = string | number | boolean | string[] | null | undefined

export function toHaveFormValues<T>(
  this: T,
  _expectedValues: Record<string, ExpectedValue>,
) {
  const context = (this as unknown) as jest.MatcherContext
  const formElement = arguments[0] as HTMLFormElement
  const expectedValues = arguments[1] as Record<string, ExpectedValue>
  ...
  return {
    ...
    message: () => {
      ...
      const matcher = `${context.isNot ? '.not' : ''}.toHaveFormValues`
