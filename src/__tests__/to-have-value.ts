...
describe('.toHaveValue', () => {
  test('handles value of text input', () => {
    ...
    const without = queryByTestId('without') as HTMLInputElement

    expect(without).toHaveValue('')
    expect(without).not.toHaveValue()
    expect(without).not.toHaveValue('foo')
    without.value = 'bar'
  ...
  test('handles value of number input', () => {
    ...
    expect(without).toHaveValue(null)
    expect(without).not.toHaveValue()
    expect(without).not.toHaveValue('10')
    without.value = '10'
    expect(without).toHaveValue(10)
  })
